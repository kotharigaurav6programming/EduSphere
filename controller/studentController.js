...
export const addTestimonialController = async (request,response)=>{
    try{
        ...
        const fileName = new Date().getTime()+filename.name;
        const pathName = path.join(__dirname.replace('\\controller',"")+'/public/testimonial/'+fileName);
        filename.mv(pathName,async(error)=>{
            try{
            if(error){
                console.log("error : ",error);
                ...
                request.body.profilePic = fileName;
            ...
            if (error.code == 11000) {
                response.render("testimonialForm.ejs", {email:request.body.email,message: message.TESTIMONIAL_ALREADY_SUBMITTED, status: status.SUCCESS });
            } else {
                response.render("notfound.ejs", { message: message.SOMETHING_WENT_WRONG, status: status.SERVER_ERROR });
            }   
        }        
